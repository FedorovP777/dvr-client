...
#ifndef MY_PROJECT_SRC_CURLHTTPCLIENT_H_
#define MY_PROJECT_SRC_CURLHTTPCLIENT_H_
/**
...
struct CurlWriteCallbackContext
{
    CurlWriteCallbackContext(
        ...
        : m_client(client), m_request(request), m_response(response), m_rateLimiter(rateLimiter), m_numBytesResponseReceived(0)
    {
    }
...
{
    CurlReadCallbackContext(
        const CurlHttpClient * client, CURL * curlHandle, HttpRequest * request, Aws::Utils::RateLimits::RateLimiterInterface * limiter)
        : m_client(client), m_curlHandle(curlHandle), m_rateLimiter(limiter), m_request(request), m_chunkEnd(false)
    {
    }
...
static size_t ReadBody(char * ptr, size_t size, size_t nmemb, void * userdata)
{
    ...
    // Needs to reserve bytes of sizeof(hex(chunk-size)) + sizeof(CRLF) +
    // sizeof(CRLF)
    if (isAwsChunked)
    {
...
void CurlHttpClient::InitGlobalState()
{
    if (!isInit)
    {
        ...
        AWS_LOGSTREAM_INFO(
            ...
            "Initializing Curl library with version: " << curlVersionData->version << ", ssl version: " << curlVersionData->ssl_version);
        isInit = true;
#ifdef AWS_CUSTOM_MEMORY_MANAGEMENT
...
CurlHttpClient::CurlHttpClient(const ClientConfiguration & clientConfig)
    ...
    , m_proxyScheme(SchemeMapper::ToString(clientConfig.proxyScheme))
...
std::shared_ptr<HttpResponse> CurlHttpClient::MakeRequest(
    ...
    Aws::Utils::RateLimits::RateLimiterInterface * writeLimiter) const
{
    ...
    // Discard Expect header so as to avoid using multiple payloads to send a http
    // request (header + body)
    ...
    if (connectionHandle)
    {
        ...
        // we only want to override the default path if someone has explicitly told
        // us to.
        if (!m_caPath.empty())
        {
        ...
        // only set by android test builds because the emulator is missing a cert
        // needed for aws services
...
#    endif // LIBCURL_VERSION_MINOR
        ...
        if (curlResponseCode != CURLE_OK && shouldContinueRequest)
        {
            ...
            AWS_LOGSTREAM_ERROR(
                CURL_HTTP_CLIENT_TAG, "Curl returned error code " << curlResponseCode << " - " << curl_easy_strerror(curlResponseCode));
        }
        ...
        else
        {
            ...
            if (request->GetMethod() != HttpMethod::HTTP_HEAD && writeContext.m_client->IsRequestProcessingEnabled()
        ...
        if (ret == CURLE_OK)
        {
            request->AddRequestMetric(
                GetHttpClientMetricNameByType(HttpClientMetricsType::DnsLatency),
        ...
        {
            request->AddRequestMetric(
                GetHttpClientMetricNameByType(HttpClientMetricsType::ConnectLatency), static_cast<int64_t>(timep * 1000));
        }
        ...
        auto curlGetInfoResult = curl_easy_getinfo(connectionHandle, CURLINFO_PRIMARY_IP,
        ...
        response->GetResponseBody().flush();
        request->AddRequestMetric(
            GetHttpClientMetricNameByType(HttpClientMetricsType::RequestLatency), (DateTime::Now() - startTransmissionTime).count());
    }
...
#endif // MY_PROJECT_SRC_CURLHTTPCLIENT_H_
